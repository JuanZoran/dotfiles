...
local widget_types = {
    icon_and_text  = require(path .. 'widgets.icon-and-text-widget'),
    icon           = require(path .. 'widgets.icon-widget'),
    arc            = require(path .. 'widgets.arc-widget'),
    horizontal_bar = require(path .. 'widgets.horizontal-bar-widget'),
    vertical_bar   = require(path .. 'widgets.vertical-bar-widget'),
}
local volume       = {}
...
local function rebuild_popup()
    spawn.easy_async(LIST_DEVICES_CMD, function(stdout)
        ...
        rows[1] = build_header_row 'SINKS'
        rows[2] = build_rows(sinks, function() rebuild_popup() end, 'sink')
        rows[3] = build_header_row 'SOURCES'
        rows[4] = build_rows(sources, function() rebuild_popup() end, 'source')
...
return function(user_args)
    ...
    local refresh_rate = args.refresh_rate or 0.25
